...
void Hashtable::qinsert(int k) {
    ...
    int indexHash = hash(k);
	for(int i = 1; i <= _size; i++) {
		if(_table[indexHash] == EMPTY) {
			_table[indexHash] = k;
			tallyProbes(i--);
			return;
		} else {
			indexHash = (indexHash + i*i) % _size;
		}
	}
...
void Hashtable::dinsert(int k) {
	...
	for(int i = 1; i <= _size; i++) {
		if(_table[indexHash] == EMPTY) {
			...
			indexHash = (indexHash + i * inc) % _size;
		}
	}
